...
source .env
#mkdir -p $CHEMIN_MONITORING_LOCAL
mkdir -p $CHEMIN_REP_NFS_LOCAL
...
mkdir -p $CHEMIN_REP_KIBANA_CERT $CHEMIN_TRUSTSTORE $CHEMIN_REP_BO_CERT $CHEMIN_REP_BO_BACKEND_CERTIFICATS
mkdir -p $CHEMIN_TRUSTSTORE/keycloak
...
mkdir -p $CHEMIN_KEYCLOAK_CERTIFICATS
...
mkdir -p $CHEMIN_MONITORING_CERTIFICATS
...
openssl pkcs12 -in $CHEMIN_REP_CERTIFICATS/client-pi.p12 -out $CHEMIN_MONITORING_CERTIFICATS/cert-prom-pi.key -nodes -password pass:$MDP_CLIENT_KEYSTORE_PI -nocerts #$MDP_SERVEUR_KEYSTORE_PI
...
mkdir -p $CHEMIN_PROMETHEUS_DATA $CHEMIN_ALERTMANAGER_DATA $CHEMIN_GRAFANA_DATA $CHEMIN_KEYCLOAK_DATA $CHEMIN_ZOOKEEPER_DATA
mkdir -p $CHEMIN_ZOOKEEPER_LOGS $CHEMIN_KAFKA_DATA
#Créer le répertoire "journalisation" et ses sous-répertoires
mkdir -p $CHEMIN_REP_JOURNALISATION
mkdir -p $CHEMIN_REP_JOURNALISATION/interface-participant
mkdir -p $CHEMIN_REP_JOURNALISATION/moteur-traitement
mkdir -p $CHEMIN_REP_JOURNALISATION/interface-pi
