...
#FIEXED
sudo cp -R config/keycloak $CHEMIN_REP_NFS_LOCAL/config/keycloak
#Copy Kafka
sudo cp -R config/kafka $CHEMIN_REP_NFS_LOCAL/config/kafka
...
#todo not working
#openssl pkcs12 -in $CHEMIN_REP_CERTIFICATS/client-pi.p12 -out $CHEMIN_MONITORING_CERTIFICATS/cert-prom-pi.key -nodes -password pass:$MDP_SERVEUR_KEYSTORE_PI -nocerts
...
#todo not working
#openssl pkcs12 -in $CHEMIN_REP_CERTIFICATS/client-pi.p12 -out $CHEMIN_MONITORING_CERTIFICATS/cert-prom-pi.crt -password pass:$MDP_SERVEUR_KEYSTORE_PI -nokeys
...
if [ -f $CHEMIN_PROMETHEUS_CONF ]
    then
        rm -fr $CHEMIN_PROMETHEUS_CONF
fi
if [ -f $CHEMIN_ALERT_CONF ]
    then
        rm -fr $CHEMIN_ALERT_CONF
fi
...
if [ -f $CHEMIN_BO_BACKEND_KEYSTORE ]
    then
        rm -fr $CHEMIN_BO_BACKEND_KEYSTORE
fi
...
if [ -f $CHEMIN_BO_BACKEND_TRUSTSTORE ]
    then
        rm -fr $CHEMIN_BO_BACKEND_TRUSTSTORE
fi
...
docker stack deploy -c $CHEMIN_FICHIER_DEPLOIEMENT_KAFKA --with-registry-auth aip_kafka
#docker compose -f $CHEMIN_FICHIER_DEPLOIEMENT_KAFKA up
#sleep 10m
sleep 1m
